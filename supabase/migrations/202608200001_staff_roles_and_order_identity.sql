...
-- Existing staff accounts keep operational access as waiters after the migration.
...
-- Keep tenant membership and role boundaries authoritative at the database layer.
...
as $$
  select exists (
    ...
    where id = auth.uid()
      and (role = 'superadmin' or (role in ('owner', 'mesero', 'cocina') and restaurant_id = target_restaurant_id))
  );
$$;
...
as $$
  select exists (
    ...
    where profile.id = auth.uid()
      and profile.role in ('owner', 'mesero', 'cocina')
...
as $$
  select exists (
    ...
    join public.restaurants as restaurant on restaurant.id = profile.restaurant_id
    where profile.id = auth.uid()
      and profile.role in ('owner', 'cocina')
      and profile.restaurant_id = target_restaurant_id
