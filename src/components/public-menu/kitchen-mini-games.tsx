...
export function KitchenMiniGames({ primaryColor }: { primaryColor: string }) {
  ...
  return <section className="mx-auto mt-4 max-w-7xl px-4 pb-10 sm:px-8">
    ...
    {active && meta ? <div className="fixed inset-0 z-[80] bg-slate-950/85 backdrop-blur-sm sm:flex sm:items-center sm:justify-center sm:p-5" role="dialog" aria-modal="true" aria-label={meta.title}><div className="flex h-[100svh] w-full flex-col overflow-hidden bg-white shadow-2xl sm:max-h-[94svh] sm:max-w-6xl sm:rounded-[2rem]">
      <header className="flex shrink-0 items-center justify-between bg-brand-navy px-4 py-3 text-white sm:px-5 sm:py-4"><div className="flex min-w-0 items-center gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-white/10"><ActiveIcon className="size-5" /></span><div className="min-w-0"><p className="truncate text-xs font-bold uppercase text-white/55">{phase === "playing" ? `${snapshot.served} clientes · combo x${snapshot.combo}` : `Récord ${best[active]} puntos`}</p><h3 className="truncate text-lg font-black sm:text-xl">{meta.title}</h3></div></div><button type="button" onClick={close} className="rounded-full bg-white/10 p-2 hover:bg-white/20" aria-label="Cerrar juego"><X /></button></header>

      {phase === "ready" ? <div style={{ backgroundImage: `linear-gradient(rgba(3,12,25,.78),rgba(3,12,25,.95)),url('/game-assets/chef-arcade-worlds.png')`, backgroundSize: "300% 100%", backgroundPosition: meta.position }} className="flex min-h-0 flex-1 flex-col items-center justify-center overflow-y-auto px-7 py-10 text-center text-white"><span className="flex size-20 shrink-0 items-center justify-center rounded-3xl bg-white/10"><ActiveIcon className="size-10 text-brand-orange" /></span><h4 className="mt-6 text-2xl font-black sm:text-3xl">{meta.title}</h4><p className="mt-3 max-w-xl text-sm leading-6 text-white/75 sm:text-base">{meta.description}</p><div className="mt-5 grid max-w-2xl gap-2 text-left text-sm font-semibold text-white/85 sm:grid-cols-3"><span className="rounded-2xl border border-white/10 bg-white/5 p-4">Tres comandas activas con paciencia independiente.</span><span className="rounded-2xl border border-white/10 bg-white/5 p-4">Procesos con tiempo real para gestionar en paralelo.</span><span className="rounded-2xl border border-white/10 bg-white/5 p-4">Combos y propinas por servir rápido y sin errores.</span></div><button type="button" onClick={begin} className="mt-7 flex min-h-14 shrink-0 items-center gap-2 rounded-2xl bg-brand-orange px-9 py-3 font-black shadow-xl"><Play className="size-5 fill-current" />Abrir restaurante</button></div> : null}
