...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const monthlyRevenue = (restaurants ?? []).reduce((sum, restaurant) => sum + planCatalog[restaurant.subscription_tier].price, 0);
