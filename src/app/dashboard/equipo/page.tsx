...
export default async function TeamPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      ...
      <form action={createTeamMemberAction} autoComplete="off" className="mt-8 rounded-3xl border bg-white p-5 shadow-sm sm:p-7">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-4"><label className="text-sm font-bold">Nombre<Input name="fullName" autoComplete="off" className="mt-2" required /></label><label className="text-sm font-bold">Usuario<Input name="staffUsername" autoComplete="off" minLength={3} placeholder="ej. ana.m" className="mt-2" required /></label><label className="text-sm font-bold">Clave<Input name="password" type="password" autoComplete="new-password" minLength={8} className="mt-2" required /></label><label className="text-sm font-bold">Rol<select name="role" defaultValue="mesero" className="mt-2 flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="mesero">Mesero</option><option value="cocina">Cocina</option></select></label></div>
      ...
      <section className="mt-6 rounded-3xl border bg-white shadow-sm">
        ...
        <div className="divide-y">{members?.length ? members.map((member) => (
          <div key={member.id} className="space-y-3 px-5 py-4">
            ...
            <form action={updateTeamMemberAction} autoComplete="off" className="grid gap-3 rounded-xl bg-slate-50 p-3 sm:grid-cols-[1fr_1fr_150px_1fr_auto] sm:items-end"><input type="hidden" name="memberId" value={member.id} /><label className="text-xs font-bold">Nombre<Input name="fullName" autoComplete="off" defaultValue={member.full_name} className="mt-1 bg-white" required /></label><label className="text-xs font-bold">Usuario<Input name="staffUsername" autoComplete="off" defaultValue={member.staff_username ?? ""} minLength={3} className="mt-1 bg-white" required /></label><label className="text-xs font-bold">Rol<select name="role" defaultValue={member.role === "cocina" ? "cocina" : "mesero"} className="mt-1 flex h-10 w-full rounded-md border border-input bg-white px-3 text-sm"><option value="mesero">Mesero</option><option value="cocina">Cocina</option></select></label><label className="text-xs font-bold">Nueva clave<Input name="password" type="password" autoComplete="new-password" minLength={8} placeholder="Opcional" className="mt-1 bg-white" /></label><Button type="submit" className="gap-2 rounded-xl bg-brand-navy"><Pencil className="size-4" />Guardar</Button></form>
          </div>
